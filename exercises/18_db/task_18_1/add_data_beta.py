...
#Функция получает список файлов. Проходим по списку, открываем каждый файл, считываем его целиком в line,
#и применяем скомпилированное регулярное выражение. Функция findall() вернёт все найденные группы в виде
#списка кортежей.
...
#line[:line.find('_')] вернёт 'sw1'
        ...
        with open(filename) as f:
            ...
            tuple_list = regex.findall(line)

#У нас задача получить имя хоста из имени файла, поэтому нам придётся извлечь каждый кортеж из списка,
#преобразовать его в список, добавить к нему имя хоста, преобразовать обратно в кортеж и добавить этот кортеж в список result			
