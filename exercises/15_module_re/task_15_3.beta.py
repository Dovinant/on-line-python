...
#Выводить в файл будем функцией print. Готовим f-строку, подставляем значениея из групп
#В конце не ставим перевод строки (символ '\n'), т.к. функция print после вывода всех значений сама добавит \n
#(меняет такое поведение print параметр 'end' )
            line_print = f"object network LOCAL_{match.group('ip')}\n host {match.group('ip')}\n nat (inside,outside) static interface service {match.group('protocol')} {match.group('port_in')} {match.group('port_out')}"
            print(line_print, file=dest)
