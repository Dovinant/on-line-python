...
#В список result поеместим получившиеся словари
    result = []
...
#Проходим по элементам кортежа. Решение не лучшее, но возможное
        for headers, values in zip(h_list, tuples):
            dict_tmp[headers] = values
        result.append(dict_tmp)
    pprint(result)
#
convert_to_dict(headers, values_list)
